...
let css = fs.readFileSync('search.css', 'utf8');
if (!css.includes('.search-frequent-header')) {
    css += `
.search-frequent-header {
    font-size: 13px;
    font-weight: 500;
...
`;
    fs.writeFileSync('search.css', css);
}
...
`;
fs.writeFileSync('search.js', jsContent);

// Bump version in HTML files to clear cache
const bumpVersion = (file) => {
    let content = fs.readFileSync(file, 'utf8');
    content = content.replace(/search\.js\?v=1003/g, 'search.js?v=1004');
    fs.writeFileSync(file, content);
};
bumpVersion('app.html');
