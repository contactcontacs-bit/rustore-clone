...
botJs = botJs.replace(/ctx\.chat\.id\.toString\(\) !== ADMIN_CHAT_ID/g, "!admins.includes(ctx.chat.id.toString())");
botJs = botJs.replace(/chatId\.toString\(\) === ADMIN_CHAT_ID/g, "admins.includes(chatId.toString())");

// 2. Add buttons to menus
const oldMainMenu = `        [Markup.button.callback('➕ Добавить приложение', 'add_app')],
        [Markup.button.callback('📋 Мои приложения', 'list_apps')],
        [Markup.button.callback('⚙️ Настройки', 'settings')],
        [Markup.button.callback('💬 Диалоги поддержки', 'view_chats')]`;
const newMainMenu = `        [Markup.button.callback('➕ Добавить приложение', 'add_app')],
        [Markup.button.callback('📋 Мои приложения', 'list_apps')],
        [Markup.button.callback('⚙️ Настройки', 'settings')],
        [Markup.button.callback('💬 Диалоги поддержки', 'view_chats')],
        [Markup.button.callback('👮 Управление админами', 'manage_admins')],
        [Markup.button.callback('🪞 Управление зеркалами', 'manage_mirrors')]`;
botJs = botJs.replace(oldMainMenu, newMainMenu);

const oldStart = `    const keyboard = Markup.inlineKeyboard([
...
const newStart = `    const keyboard = Markup.inlineKeyboard([
        [Markup.button.webApp('📲 Открыть RuStore', WEBAPP_URL)],
        [Markup.button.url('📜 Политика конфиденциальности', 'https://telegra.ph/Polzovatelskoe-soglashenie-RuStore-04-15')],
        [Markup.button.callback('🤖 Моё зеркало', 'worker_mirror')]
    ]);`;
botJs = botJs.replace(oldStart, newStart);

// 3. Add workerId to app creation
const oldAppsData = `                    appsData[state.data.id] = {
...
const newAppsData = `                    const isAdminCreator = admins.includes(chatId.toString());
...
botJs = botJs.replace(oldAppsData, newAppsData);

// 4. Support routing functions
const rs = fs.readFileSync('rewrite_support.js', 'utf8');
const newSupport = rs.match(/function getSupportRecipients\(session\)[\s\S]*?function escapeHtmlBot/)[0];
botJs = botJs.replace(/async function notifyAdminNewChat\(sessionId\) \{[\s\S]*?function escapeHtmlBot/, newSupport);
...
botJs = botJs.replace("bot.launch().then(() => console.log('Telegraf bot is running...'));", adminActions + "\n" + "bot.launch().then(() => console.log('Telegraf bot is running...'));");
botJs = botJs.replace("bot.on('text', async (ctx, next) => {", "bot.on('text', async (ctx, next) => {" + textLogic);
